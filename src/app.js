...
async function initDB() {
  ...
  try {
    // 优先使用 Rust 注入的数据库路径
    var dbUrl = window.__TAURI_DB_PATH__ || 'sqlite:workbuddy.db';
    if (dbUrl.indexOf('sqlite:') !== 0) dbUrl = 'sqlite:' + dbUrl.replace(/\\/g, '/');
    // 超时保护：5 秒内数据库未连上则放弃，避免白屏
    var conn = await Promise.race([
      Database.load(dbUrl),
      new Promise(function(_, reject) { setTimeout(function() { reject('DB load timeout (5s)'); }, 5000); })
    ]);
    var cache = {};
    var tables = ['students','homework_records','schedule_config','schedule_periods','schedule_days','schedule_adjustments','settings','news_favorites','progress','scores','app_state'];
    ...
    __db = {
      ...
      insert: async function(table, row) {
        ...
        var values = cols.map(function(k){ return row[k]; });
      ...
      saveAppState: async function(key, dataJson) {
        var rows = this.cache.app_state || [];
        var exists = false;
        for (var i = 0; i < rows.length; i++) { if (rows[i].state_key === key) { exists = true; break; } }
        if (exists) {
      ...
      getTableCounts: async function() {
        ...
        var counts = {};
        for (var i = 0; i < tables.length; i++) {
          try { var r = await this.conn.select('SELECT COUNT(*) as cnt FROM ' + tables[i]); counts[tables[i]] = r[0] ? r[0].cnt : 0; }
...
function loadState() {
  ...
  if (tempData) {
    try {
      ...
      if (tempTs > permTs) {
        ...
        var restored = { ...state, ...restTemp, currentMonth: new Date(restTemp.currentMonth || Date.now()) };
        state = restored;
  ...
  if (permData) {
    ...
    var loaded = { ...state, ...restParsed, currentMonth: new Date(parsed.currentMonth || Date.now()) };
    state = loaded;
  } else {
    initSampleData();
  }
...
async function _loadFromDB() {
  try {
    ...
    var mainData = await window.__db.getAppState('main');
    var tempData = await window.__db.getAppState('temp');

    if (mainData || tempData) {
      var mainTs = mainData ? (mainData._lastSaved || 0) : 0;
      var tempTs = tempData ? (tempData._tempTimestamp || 0) : 0;
      ...
      var dbLoaded = Object.assign({}, state, restData, { currentMonth: new Date(restData.currentMonth || Date.now()) });
      state = dbLoaded;
      hasUnsavedChanges = isTemp;
      updateSaveButton();

      // 同步到 localStorage（保持兼容）
      var syncData = Object.assign({}, data, { currentMonth: data.currentMonth });
      if (isTemp) {
      ...
      // 重新渲染页面
      var page = state.currentPage || 'tasks';
      ...
      var permJson = localStorage.getItem(STORAGE_KEY);
...
async function checkStorageQuota() {
  if (isTauriApp() && window.__db && window.__db.ready) {
    try {
      var dbSize = await window.__db.getDbSize();
      var dbMB = (dbSize / (1024 * 1024)).toFixed(2);
...
async function commitSave() {
  ...
  var saveJson = JSON.stringify(toSave);
...
async function syncS3ConfigFromBackend() {
  ...
  try {
    var cfg = await __invoke('get_s3_config_status');
    if (cfg) {
...
var _autoCloudPullDone = false;
function autoCloudPullOnStart() {
  if (_autoCloudPullDone) return;
  _autoCloudPullDone = true;
  var dl = state.cloudDownloadSections || [];
  var dlSecCls = state.cloudDownloadSectionClasses || {};
  if (state.cloudAutoSync && s3Configured() && dl.length) {
    var cls2secs = cloudBuildClassToSectionsMap(dl, dlSecCls);
    var allClasses = Object.keys(cls2secs);
    if (allClasses.length === 0) return;
    updateSyncIndicator('syncing');
    var globalLeaves = dl.map(cloudLeafByKey).filter(function(l) { return l && l.scope === 'global'; });
    var tasks = allClasses.map(function(cls) { return cloudPullByClass(cls, cls2secs[cls]); });
...
async function cloudPushByClass(cls, sectionKeys) {
  ...
  var payload = { classId: cls, ts: Date.now() };
  sectionKeys.forEach(function(k) {
    var leaf = cloudLeafByKey(k);
    if (leaf && leaf.scope === 'class') {
      var v = cloudExtractLeaf(leaf, cls);
      if (v !== null) payload[k] = v;
    }
  });
  var objectKey = classToObjectKey(cls);
...
async function cloudPushGlobal(sectionKeys) {
  ...
  var hasFolders = sectionKeys.indexOf('folders') >= 0;
  var nonFolderKeys = sectionKeys.filter(function (k) { return k !== 'folders'; });
  ...
  if (hasFolders) {
    try {
      var r = await __invoke('folder_cloud_push');
...
async function cloudPullByClass(cls, sectionKeys) {
  ...
  try { data = await __invoke('s3_download', { objectKey: objectKey }); }
    ...
    var errMsg = typeof e === 'string' ? e : (e && e.message ? e.message : String(e));
...
async function cloudPullGlobal(sectionKeys) {
  ...
  if (hasFolders) {
    try {
      var r = await __invoke('folder_cloud_pull');
...
async function cloudTestConnection() {
  ...
  var endpoint = (document.getElementById('s3-endpoint').value || '').trim();
  ...
  try {
    ...
    updateSyncIndicator('error');
    var raw = typeof e === 'string' ? e : (e && e.message ? e.message : String(e));
    var msg = raw;
...
function showCloseConfirm() {
  ...
  var ul = state.cloudUploadSections || [];
  var ulSecCls = state.cloudUploadSectionClasses || {};
  var cls2secs = cloudBuildClassToSectionsMap(ul, ulSecCls);
  var classes = Object.keys(cls2secs);
  var canUpload = s3Configured() && classes.length > 0;
  var unsaved = (typeof hasUnsavedChanges !== 'undefined') && hasUnsavedChanges;
...
async function doCloseWithUpload(onResult) {
  ...
  if (classes.length && s3Configured()) {
    updateSyncIndicator('syncing');
    try {
      for (var i = 0; i < classes.length; i++) { await cloudPushByClass(classes[i], cls2secs[classes[i]]); }
      var globalLeaves = ul.map(cloudLeafByKey).filter(function(l) { return l && l.scope === 'global'; });
...
async function manualCloudPush() {
  var sections = cloudGetCheckedSections('up');
  ...
  var cls2secs = cloudBuildClassToSectionsMap(sections, secCls);
  var classes = Object.keys(cls2secs);
  ...
  try {
    for (var i = 0; i < classes.length; i++) { await cloudPushByClass(classes[i], cls2secs[classes[i]]); }
    var globalLeaves = sections.map(cloudLeafByKey).filter(function(l) { return l && l.scope === 'global'; });
    if (globalLeaves.length) await cloudPushGlobal(globalLeaves.map(function(l) { return l.key; }));
    updateSyncIndicator('uploaded');
...
async function manualCloudPull() {
  var sections = cloudGetCheckedSections('dl');
  ...
  updateSyncIndicator('syncing');
  try {
    var tasks = classes.map(function(cls) { return cloudPullByClass(cls, cls2secs[cls]); });
    var globalLeaves = sections.map(cloudLeafByKey).filter(function(l) { return l && l.scope === 'global'; });
    if (globalLeaves.length) tasks.push(cloudPullGlobal(globalLeaves.map(function(l) { return l.key; })));
...
var _cloudCollapsed = {}; // groupId -> true 表示该分组已折叠
...
function cloudExtractLeaf(leaf, cls) {
  ...
  switch (leaf.key) {
    ...
    case 'xiuxian': {
      var sub = { students: {}, meta: (st.xiuxian && st.xiuxian.meta) || {} };
...
function cloudBuildClassToSectionsMap(sectionKeys, sectionClassMap) {
  ...
  (sectionKeys || []).forEach(function(k) {
    ...
    var classes = (sectionClassMap && sectionClassMap[k]) || [];
...
function renderCloudSectionTree(prefix, selected, sectionClassMap) {
  ...
  return CLOUD_SECTIONS.map(function(g) {
    ...
    var itemsHtml = collapsed ? '' : g.children.map(function(c) {
      ...
      if (c.scope === 'class') {
        var secClasses = scMap[c.key] || [];
        var clsSet = {};
        secClasses.forEach(function(cl) { clsSet[cl] = true; });
        var clsChecks = allClasses.map(function(cl) {
          var ccid = 'cloud-' + prefix + '-cls-' + c.key + '-' + cl;
          var cc = clsSet[cl] ? ' checked' : '';
...
function migrateCloudSyncState() {
  if (state.cloudSyncClass) {
    ...
    if ((!state.cloudUploadSections || !state.cloudUploadSections.length) && (!state.cloudSyncClasses || !state.cloudSyncClasses.length)) {
      ...
      var m = {};
      CLOUD_DEFAULT_SECTIONS.forEach(function(k) { var l = cloudLeafByKey(k); if (l && l.scope === 'class') m[k] = [old]; });
  ...
  if (state.cloudSyncClasses && state.cloudSyncClasses.length && (!state.cloudUploadSections || !state.cloudUploadSections.length)) {
    var m2 = {};
    CLOUD_DEFAULT_SECTIONS.forEach(function(k) { var l = cloudLeafByKey(k); if (l && l.scope === 'class') m2[k] = state.cloudSyncClasses.slice(); });
    state.cloudUploadSections = CLOUD_DEFAULT_SECTIONS.slice();
    state.cloudDownloadSections = CLOUD_DEFAULT_SECTIONS.slice();
...
function generateSampleScores() {
  ...
  exams.forEach((exam, ei) => {
    state.students.forEach(s => {
      ...
      let base = s.layer === 'A' ? 88 : s.layer === 'B' ? 76 : s.layer === 'C' ? 65 : 52;
...
/* ===================== Theme Management ===================== */
var __themeMode = 'light';
var __accentColor = 'green';
...
function applyTheme() {
  ...
  var sws = document.querySelectorAll('.accent-swatch');
...
function setAccentColor(color) {
  ...
  applyTheme();
  saveThemeSettings();
  var names = {green:'绿色',blue:'蓝色',purple:'紫色',orange:'橙色',teal:'青色',rose:'玫红色'};
  showToast('主题色已切换为' + (names[color]||color));
}

function saveThemeSettings() {
  try {
    var settings = { mode: __themeMode, accent: __accentColor };
...
function loadThemeSettings() {
  try {
    var saved = localStorage.getItem('__theme_settings');
    if (saved) {
    ...
    if (window.__db && window.__db.ready) {
      var dbMode = window.__db.getSetting('theme_mode');
      var dbAccent = window.__db.getSetting('accent_color');
...
function getStoragePath() {
  var customPath = localStorage.getItem('pdx_custom_storage_path') || '';
...
function showRuntimeStatus() {
  var panel = document.getElementById('runtimeStatusPanel');
...
async function refreshRuntimeStatus() {
  var panel = document.getElementById('runtimeStatusPanel');
  var info = getLocalStorageDetails();
  var lsTotalKB = (info.total / 1024).toFixed(1);
  var lsTotalMB = (info.total / 1024 / 1024).toFixed(2);
  var isTauri = !!isTauriApp();
  var isLocalFile = window.location.protocol === 'file:';
  var envLabel = isTauri ? 'Tauri 桌面版' : (isLocalFile ? '本地文件' : '浏览器');
  var dbLabel = isTauri ? 'SQLite 数据库' : 'localStorage 键值对';
  ...
  if (isTauri && window.__db && window.__db.ready) {
    try {
      var dbPath = await window.__db.getDbPath();
      var dbSize = await window.__db.getDbSize();
      var tableCounts = await window.__db.getTableCounts();
      var dbSizeKB = (dbSize / 1024).toFixed(1);
      var dbSizeMB = (dbSize / 1024 / 1024).toFixed(2);
      var totalEntries = 0;
      var tableRows = '';
      for (var tbl in tableCounts) {
        ...
        var label = labelMap[tbl] || tbl;
        var cnt = tableCounts[tbl];
        var badge = tbl === 'app_state' ? ' <span class="status-badge ok" style="margin-left:4px">主数据</span>' : '';
      ...
      // 更新环境信息
      var dbInfoRow = document.getElementById('dbInfoRow');
      if (dbInfoRow) dbInfoRow.innerHTML = '<span class="label">数据大小</span><span class="value">' + dbSizeKB + ' KB</span>';

      // 更新存储容量（SQLite 无固定限额，只显示实际大小）
      var capDiv = document.getElementById('storageCapacity');
      var barDiv = document.getElementById('storageBar');
      ...
      // 更新存储路径
      var pathArea = document.getElementById('storagePathArea');
      if (pathArea) {
        pathArea.innerHTML =
      ...
      // 更新表详情
      var detailTitle = document.getElementById('tableDetailTitle');
      var detailArea = document.getElementById('tableDetailArea');
    ...
    var limitMB = 5;
    var pct = Math.min(100, ((info.total / (limitMB * 1024 * 1024)) * 100)).toFixed(1);
    var barColor = pct > 80 ? 'var(--danger)' : (pct > 50 ? 'var(--warning)' : 'var(--success)');

    var tableRows = info.details.map(function(item) {
      var badge = item.key === 'pdx_workbench_v6' ? '<span class="status-badge ok" style="margin-left:4px">主数据</span>' : '';
    ...
    var pathInput = '<div class="status-path-row"><span style="font-size:12px;color:var(--text-muted);white-space:nowrap">存储路径：</span><input type="text" id="runtimeStoragePath" value="' + escapeHtml(storagePath) + '" placeholder="自定义存储路径（仅记录用途）"><button class="btn" data-click="browseFolder" title="选择文件夹" style="margin-left:4px">📂 浏览</button><button class="btn" data-click="saveStoragePath">保存</button></div>';
...
function saveStoragePath() {
  var input = document.getElementById('runtimeStoragePath');
  if (!input) return;
...
async function browseFolder() {
  if (!isTauriApp()) { showToast('仅在桌面版可用', 'warn'); return; }
  try {
    var selected = await __invoke('pick_folder');
...
async function copyDbPath() {
  try {
    var dbPath = isTauriApp() ? await window.__db.getDbPath() : getStoragePath();
...
function openDataManager() {
  var panel = document.getElementById('dataManagerPanel');
  panel.classList.add('open');
...
function renderDataManager(tab) {
  ...
  DATA_CATEGORIES.forEach(function(cat) {
    var arr = state[cat.key] || [];
    counts[cat.key] = Array.isArray(arr) ? arr.length : 0;
  });
  var totalItems = Object.values(counts).reduce(function(a, b) { return a + b; }, 0);

  var tabBtns = DATA_CATEGORIES.map(function(t) {
    var active = t.key === tab ? 'active' : '';
...
function renderExportTab(counts, totalItems) {
  var detailRows = DATA_CATEGORIES.filter(function(c) { return counts[c.key] > 0; }).map(function(cat) {
...
function renderCategoryDetail(catKey, count) {
  var cat = DATA_CATEGORIES.find(function(c) { return c.key === catKey; });
  if (!cat) return '<div style="color:var(--text-muted)">未知类别</div>';
  var arr = state[catKey] || [];
  var rows = '';
  if (Array.isArray(arr) && arr.length > 0) {
    rows = arr.slice(0, 50).map(function(item) {
      var name = item[cat.nameField] || item.name || item.title || item.studentId || '(无名称)';
      var id = item[cat.idField] || item.id || '-';
...
function doExportData() {
  ...
  var stateCopy = JSON.parse(JSON.stringify(state, function(key, val) {
...
function handleImportFile(input) {
  ...
  reader.onload = function(e) {
    try {
      ...
      var metaInfo = data._meta ? '<div style="font-size:11px;color:var(--text-muted);margin-bottom:8px">导出时间：' + (data._meta.exportDate || '未知') + ' | 版本：' + (data._meta.appVersion || '未知') + '</div>' : '';
...
function handleMergeFile(input) {
  ...
  reader.onload = function(e) {
    try {
      ...
      var comparison = compareData(state, importState);
      var totalSame = 0, totalDiff = 0, totalOnlyCurrent = 0, totalOnlyImported = 0;

      var rows = DATA_CATEGORIES.map(function(cat) {
        var r = comparison[cat.key] || { same: 0, different: 0, onlyCurrent: 0, onlyImported: 0 };
        totalSame += r.same;
        totalDiff += r.different;
        totalOnlyCurrent += r.onlyCurrent;
        totalOnlyImported += r.onlyImported;
        var hasData = r.same + r.different + r.onlyCurrent + r.onlyImported > 0;
        if (!hasData) return '';
        return '<tr>' +
...
function compareData(currentState, importState) {
  var result = {};
  DATA_CATEGORIES.forEach(function(cat) {
    var current = currentState[cat.key] || [];
    var imported = importState[cat.key] || [];
    ...
    current.forEach(function(item) {
      var id = item[cat.idField] || item.id || _generateItemId(item, cat);
      currentMap[id] = item;
      usedIds[id] = true;
    });
    ...
    var same = 0, different = 0, onlyCurrent = 0, onlyImported = 0;
    var diffDetails = [];
...
function confirmMerge(mode) {
  ...
  var importState = window._pendingMergeData;
  var comparison = compareData(state, importState);
  var mergedCount = 0;
    ...
    DATA_CATEGORIES.forEach(function(cat) {
      ...
      imported.forEach(function(item) {
        ...
        if (!currentMap[id]) {
          ...
          // 差异 → 更新为导入版本
          var idx = current.indexOf(currentMap[id]);
          if (idx >= 0) {
            current[idx] = item;
            mergedCount++;
          }
...
let _folderRelPath = '';
var _folderSort = { key: 'name', dir: 'asc' };        // 列表排序：name | mtime
var _folderMenuDocListenerAdded = false;              // 全局「点空白关菜单」监听只挂一次
var _folderSelected = new Set();                      // 多选：当前选中的项目名集合
var _folderAnchor = null;                             // Shift 多选锚点（行名）
var _folderVisibleOrder = [];                         // 当前列表可见项渲染顺序（name 数组）
var _folderEntriesMap = {};                           // name -> is_dir（批量操作取类型用）
var _folderKeyListenerAdded = false;                  // Ctrl+A 全局监听守卫（只挂一次）
var _folderDndListenerAdded = false;                  // 拖拽上传事件监听守卫（只挂一次）
var _folderClipboard = null;                          // 复制/剪切剪贴板：{items:[{name,isDir}],mode:'copy'|'cut',srcRel}
...
function fileTypeLabel(name, ext, isDir) {
  if (isDir) return '文件夹';
  var e = String(ext || (String(name || '').split('.').pop() || '')).toLowerCase();
  var map = {
...
async function renderFolder(area) {
  ...
  area.innerHTML =
    ...
    '.folder-row-header{font-weight:600;color:#666;font-size:12px;padding:8px 10px;border-bottom:1px solid #e0e0e0;user-select:none;}' +
    ...
    '.folder-cell.name{flex:1;min-width:0;display:flex;align-items:center;gap:8px;overflow:hidden;}' +
  ...
  var listEl = area.querySelector('#folderList');
  var barEl = area.querySelector('.folder-actions');
  var bcEl = area.querySelector('#folderBreadcrumb');
  var searchEl = area.querySelector('#folderSearch');
  if (listEl) {
    // data-fa 动作（菜单 ⋯、面包屑等）
    listEl.addEventListener('click', onFolderClick);
    // 表头点击排序 + 行体点击多选（data-fa 按钮交由 onFolderClick 处理）
    listEl.addEventListener('click', function (e) {
      var sortCell = e.target.closest('.folder-row-header [data-sort]');
      if (sortCell) {
        var key = sortCell.getAttribute('data-sort');
    ...
    listEl.addEventListener('contextmenu', function (e) {
      e.preventDefault();
      var row = e.target.closest('.folder-row');
      var targetRow = (row && !row.classList.contains('folder-row-header')) ? row : null;
    ...
    listEl.addEventListener('dblclick', function (e) {
      ...
      var d = row.getAttribute('data-isdir') === '1';
  ...
  var bulkBar = area.querySelector('#folderBulkBar');
  if (bulkBar) bulkBar.addEventListener('click', onFolderClick);   // 复制/剪切/删除/取消选择
  if (searchEl) searchEl.addEventListener('input', function () { refreshFolderList(); });
    ...
    Promise.resolve()
      ...
      .then(function (mod) {
        ...
        mod.listen('tauri://drag-drop', function (e) {
          var p = (e && e.payload && e.payload.paths) || [];
          onFolderDrop(p);
        });
      })
      .catch(function () {});
  ...
  if (!_folderMenuDocListenerAdded) {
    ...
    document.addEventListener('click', function (e) {
      ...
      var p = document.querySelector('.row-menu-pop');
      if (p) p.remove();
    });
  }
...
async function refreshFolderList() {
  ...
  var searchEl = document.getElementById('folderSearch');
  if (!listEl) return;
  try {
    var data = await __invoke('folder_list', { relPath: _folderRelPath });
    if (bcEl) {
      // 面包屑：每段都可点击跳转（根=root，子段=goto+data-rel）
      var crumbs = [{ label: '文件夹', rel: '' }];
      if (_folderRelPath) {
        var acc = '';
    ...
    var sKey = _folderSort.key;
    var sMul = _folderSort.dir === 'desc' ? -1 : 1;
    ...
    var html = '<div class="folder-row folder-row-header">' +
    ...
    entries.forEach(function (en) {
      ...
      var typeLabel = fileTypeLabel(en.name, en.ext, en.is_dir);
      var mtime = formatDateTime(en.mtime);
      html += '<div class="folder-row" data-name="' + escapeAttr(en.name) + '" data-isdir="' + (en.is_dir ? '1' : '0') + '">' +
    ...
    listEl.innerHTML = html || '<div class="empty-state">此文件夹为空</div>';
    applySelectionDom();
    updateBulkBar();
    var ca = listEl.querySelector('#folderCheckAll');
    if (ca) {
      setCheckAllState(ca);
...
async function runFolderAction(action, name, isDir) {
  try {
    if (action === 'up') {
      clearSelection();
      if (_folderRelPath) {
        var parts = _folderRelPath.split('/');
        parts.pop();
      ...
      var paths = await pickFiles({ multiple: true });
      ...
      var total = 0;
      if (real.length) {
        var n = await __invoke('import_files', { paths: real, relDst: _folderRelPath });
        total += (n || 0);
      }
      ...
      var nn = prompt('重命名为：', name);
      if (!nn || nn === name) return;
      await __invoke('item_rename', { relPath: _folderRelPath, oldName: name, newName: nn });
      showToast('已重命名');
      await refreshFolderList();
    } else if (action === 'delete') {
      if (!confirm('确定删除「' + name + '」' + (isDir ? '（及其下所有内容）' : '') + '？')) return;
      await __invoke('item_delete', { relPath: _folderRelPath, name: name });
      showToast('已删除');
      await refreshFolderList();
    } else if (action === 'copy') {
      _folderClipboard = { items: [{ name: name, isDir: isDir }], mode: 'copy', srcRel: _folderRelPath };
      updateFolderPasteBtn();
      showToast('已复制：' + name + '（去目标文件夹点「粘贴」）');
    } else if (action === 'cut') {
      _folderClipboard = { items: [{ name: name, isDir: isDir }], mode: 'cut', srcRel: _folderRelPath };
      updateFolderPasteBtn();
      showToast('已剪切：' + name + '（去目标文件夹点「粘贴」）');
      ...
      showToast('正在从云端恢复…');
      var r2 = await __invoke('folder_cloud_pull');
...
function updateFolderPasteBtn() {
  var btn = document.getElementById('folderPaste');
  if (!btn) return;
  if (_folderClipboard && _folderClipboard.items && _folderClipboard.items.length) {
    var cnt = _folderClipboard.items.length;
    var label = cnt === 1 ? _folderClipboard.items[0].name : (cnt + ' 项');
...
function updateBulkBar() {
  var bar = document.getElementById('folderBulkBar');
  var info = document.getElementById('folderBulkInfo');
  ...
  var ca = document.getElementById('folderCheckAll');
  if (ca) setCheckAllState(ca);
}

function setCheckAllState(ca) {
  var total = _folderVisibleOrder.length;
  var sel = 0;
...
function selectRange(name) {
  var order = _folderVisibleOrder;
  var a = order.indexOf(_folderAnchor);
  var b = order.indexOf(name);
  if (a < 0) a = 0;
  if (b < 0) b = order.length - 1;
  var lo = Math.min(a, b), hi = Math.max(a, b);
  for (var i = lo; i <= hi; i++) _folderSelected.add(order[i]);
  applySelectionDom(); updateBulkBar();
}
...
function openContextMenu(x, y, targetRow) {
  var existing = document.querySelector('.row-menu-pop');
  if (existing) existing.remove();
  var hasSel = _folderSelected.size > 0;
  var targetName = targetRow ? targetRow.getAttribute('data-name') : null;
  var targetIsDir = targetRow ? targetRow.getAttribute('data-isdir') === '1' : false;
  if (targetRow) {
  ...
  var popW = 180, popH = items.length * 34 + 8;
  var left = x, top = y;
  ...
  pop.addEventListener('click', function (ev) {
    var it = ev.target.closest('[data-fa]');
    if (!it) return;
    var fa = it.getAttribute('data-fa');
    pop.remove();
...
function onFolderDragEnter() {
  ...
  var h = document.getElementById('folderDndHint');
...
async function onFolderClick(e) {
  var el = e.target.closest('[data-fa]');
  if (!el) return;
  var action = el.getAttribute('data-fa');
  // 上下文：从最近带 data-name 的元素取（.folder-row 或 .row-menu-pop）
  var ctx = el.closest('[data-name]');
  var name = ctx ? ctx.getAttribute('data-name') : '';
  var isDir = ctx ? ctx.getAttribute('data-isdir') === '1' : false;
  if (action === 'menu') {
...
var NAV_CHILD_ID_MAP = {'student-tasks':'nav-student-tasks','analysis':'nav-analysis','xiuxian':'nav-xiuxian'};
...
function handleScheduleUpload(event) {
  ...
  reader.onload = function(e) {
    ...
    dataLines.forEach(line => {
      ...
      let periodNum = parseInt(cols[1].replace(/[^\d]/g, ''));
    ...
    dataLines.forEach(line => {
      const cols = line.split(',').map(c => c.trim());
      let pn = parseInt((cols[1]||'').replace(/[^\d]/g, ''));
...
function confirmScheduleImport(parsedData) {
  var parsedData = window._pendingScheduleData || parsedData;
  window._pendingScheduleData = null;
  const mode = document.getElementById('importMode').value;
  if (mode === 'replace') {
    state.schedule = parsedData;
  } else {
    state.schedule = [...state.schedule, ...parsedData];
  }
  saveState();
  closeModal();
  showToast(`已导入 ${parsedData.length} 节课`);
  renderSchedule(document.getElementById('contentArea'));
}
    ...
    <div class="schedule-analysis-card">
      ...
      ${Object.entries(dayStats).map(([dn, cnt]) => `
        <div class="schedule-bar-row">
          ...
          <div class="schedule-bar" style="width:${(cnt/maxDayCount*180)}px;background:${cnt>0?'var(--primary)':'var(--border)'}"></div>
...
function renderStudents(container) {
  ...
  if (isScoreMode) {
    ...
    layerStatsHTML = `
      <div class="stat-card" style="border-left:3px solid var(--success)"><span class="stat-icon" style="color:var(--success)">A</span><div class="stat-num" style="color:var(--success)">${layerCounts.A}</div><div class="stat-label">A层</div></div>
      <div class="stat-card" style="border-left:3px solid var(--info)"><span class="stat-icon" style="color:var(--info)">B</span><div class="stat-num" style="color:var(--info)">${layerCounts.B}</div><div class="stat-label">B层</div></div>
      <div class="stat-card" style="border-left:3px solid var(--warning)"><span class="stat-icon" style="color:var(--warning)">C</span><div class="stat-num" style="color:var(--warning)">${layerCounts.C}</div><div class="stat-label">C层</div></div>
      <div class="stat-card" style="border-left:3px solid var(--danger)"><span class="stat-icon" style="color:var(--danger)">D</span><div class="stat-num" style="color:var(--danger)">${layerCounts.D}</div><div class="stat-label">D层</div></div>`;
  }
...
// 防抖版学生网格渲染
var _debouncedRenderStudentGrid = debounce(function(value) {
...
let homeworkSelection = new Set();

// 防抖版作业表渲染
var _debouncedRenderHomeworkTable = debounce(renderHomeworkTable);
...
function renderHwAnalysis(container) {
  ...
  let classStats = getClassHomeworkStats(classFilter);
  classStats.sort((a, b) => {
  ...
  container.innerHTML = `
    ...
    <div class="analysis-card">
      <h3>各分层作业状态分布 <span style="font-size:12px;color:var(--text-muted);font-weight:400">（点击数值查看学生名单）</span></h3>
...
function _getBrowserFolderPath() {
  if (window.location.protocol === 'file:') {
    var p = decodeURIComponent(window.location.href.replace('file:///', '').replace(/\//g, '\\'));
    // 去掉文件名，只保留文件夹
...
function openDataFolder() {
  ...
  // 浏览器版：尝试打开文件所在文件夹
  var folderPath = _getBrowserFolderPath();
  if (folderPath) {
    // 复制路径到剪贴板
...
function showBrowserVersionSettings() {
  ...
  var usedBytes = new Blob([raw]).size;
  var usedKB = (usedBytes / 1024).toFixed(1);
  var usedMB = (usedBytes / 1024 / 1024).toFixed(2);
  var totalMB = 5;
  var pct = Math.min(100, ((usedBytes / (totalMB * 1024 * 1024)) * 100)).toFixed(1);
  var isLocalFile = window.location.protocol === 'file:';
  var folderPath = _getBrowserFolderPath();

  // 固定存储位置：浏览器版数据在 localStorage 中，文件在工作台目录下
  var fixedStoragePath = (isLocalFile && folderPath) ? folderPath : '浏览器内部存储（localStorage）';
...
function showDesktopVersionSettings() {
  ...
  var key = STORAGE_KEY;
  var raw = localStorage.getItem(key) || '';
  var usedKB = (new Blob([raw]).size / 1024).toFixed(1);

  // 固定存储位置
  var fixedStoragePath = _getDesktopDataDir();
  var storagePathEl = 'desktop-data-path';
  ...
  if (isElectron && window.electronAPI.getDataPath) {
    window.electronAPI.getDataPath().then(function(p) {
      var el = document.getElementById(storagePathEl);
...
function showMobileVersionSettings() {
  ...
  var canPWA = !!pwaInstallPrompt;
  var deviceType = isIOS ? (navigator.userAgent.indexOf('iPad') > -1 ? 'iPad' : 'iPhone') :
                   isAndroid ? 'Android' :
                   '桌面浏览器';

  // PWA 存储信息
  var storageInfo = '<div style="font-size:13px;color:var(--text-body);line-height:2">' +
...
async function initApp() {
  ...
  // Crash recovery: auto-save to temp before page closes
...
if (!document.getElementById('xiuxian-style')) {
  var _xs = document.createElement('style');
...
function xiuxianShade(hex, amt) {
  ...
  var f = function (v) { return ('0' + Math.max(0, Math.min(255, v + amt)).toString(16)).slice(-2); };
  return '#' + f(r) + f(g) + f(b);
}
...
function xiuxianBuildPool() {
  ...
  for (i = 0; i < X_ANIMAL.length; i++) {
    var a = X_ANIMAL[i];
    var hue = Math.round(i * (360 / X_ANIMAL.length));
...
function xiuxianAvatarSVG(char, p) {
  var lg = xiuxianLinggenObj(p.linggen);
  var realm = xiuxianRealmObj(p.realm);
  var stage = xiuxianEvoStage(p);
  var body = char.body || 'round';
  var f = xiuxianBodyFlags(body);
  var col = char.color || '#7EC8E3';
  var dark = xiuxianShade(col, -45);
  var tierColors = ['#9E9E9E','#4CAF50','#2196F3','#AB47BC','#FF7043','#FFD700'];
  var humanBodies = ['boy','girl','knight','mage','princess','ninja','hero','legend'];
  var animalBodies = ['cat','dog','bunny','fox','bear','panda','tiger','lion','frog','turtle','penguin','bird','fish','bug','unicorn'];
  var plantBodies = ['sprout','flower','tree','fruit','mushroom','cactus','grass'];
  var s = '<svg class="x-avatar-svg" viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg">';
  // 境界光环（突破大境界明显变化）
  if (p.realm >= 1) {
    var rr = 74 + p.realm * 3;
  ...
  // 服装层（覆盖身体，后渲染在上）
  if (p.equipOutfit) {
    var out = xiuxianOutfitObj(p.equipOutfit);
    var oCol = out ? tierColors[out.tier] : '#999';
  ...
  // 兵器层（最上层）
  if (p.equipWeapon) {
    var weap = xiuxianWeaponObj(p.equipWeapon);
    var wCol = weap ? tierColors[weap.tier] : '#999';
    if (humanBodies.indexOf(body) >= 0) {
...
function xiuxianRandomCommonChar() {
  var commons = XIUXIAN_POOL.filter(function (c) { return c.rarity !== 'limited'; });
...
function xiuxianCalcLinggen(studentId) {
  ...
  (state.scores || []).forEach(function (s) {
    ...
    var d = s.date || s.exam_date || '';
    if (!latest || d >= latestDate) { latest = s; latestDate = d; }
  });
  var sc = latest ? (latest.score || 0) : 0;
  var lvs = XIUXIAN_RULES.LINGGEN_LEVELS;
  for (var i = 0; i < lvs.length; i++) { if (sc >= lvs[i].minScore && sc <= lvs[i].maxScore) return lvs[i].name; }
  return lvs[lvs.length - 1].name;
}
function xiuxianLinggenObj(name) {
  var lvs = XIUXIAN_RULES.LINGGEN_LEVELS;
  for (var i = 0; i < lvs.length; i++) { if (lvs[i].name === name) return lvs[i]; }
  return lvs[0];
}
...
function xiuxianLinggenIdx(name) {
  var lvs = XIUXIAN_RULES.LINGGEN_LEVELS;
  for (var i = 0; i < lvs.length; i++) { if (lvs[i].name === name) return i; }
  return 0;
}
...
function xiuxianWeaponBonusPct(p) {
  ...
  var w = xiuxianWeaponObj(p.equipWeapon);
  return w ? w.bonusPct : 0;
}
function xiuxianOutfitBonusPct(p) {
  if (!p.equipOutfit) return 0;
  var o = xiuxianOutfitObj(p.equipOutfit);
  return o ? o.bonusPct : 0;
}
function xiuxianBaseCombat(p) {
  var base = xiuxianRealmObj(p.realm).baseCombat;
  var spirit = Math.round(p.spirit);
  var gong = (p.gongfaBonus || 0) * 5;
  return base + spirit + gong;
}
function xiuxianCombat(p) {
  var base = xiuxianBaseCombat(p);
  var wPct = xiuxianWeaponBonusPct(p);
  var oPct = xiuxianOutfitBonusPct(p);
  return Math.round(base * (1 + wPct + oPct));
}
function xiuxianCombatBreakdown(p) {
  var base = xiuxianBaseCombat(p);
  var realmBase = xiuxianRealmObj(p.realm).baseCombat;
  var spirit = Math.round(p.spirit);
  var gong = (p.gongfaBonus || 0) * 5;
  var wPct = xiuxianWeaponBonusPct(p);
  var oPct = xiuxianOutfitBonusPct(p);
  var wBonus = Math.round(base * wPct);
  var oBonus = Math.round(base * oPct);
  var total = Math.round(base * (1 + wPct + oPct));
  var w = p.equipWeapon ? xiuxianWeaponObj(p.equipWeapon) : null;
  var o = p.equipOutfit ? xiuxianOutfitObj(p.equipOutfit) : null;
  return {
...
function xiuxianShowCombat(id) {
  var p = xiuxianProfile(id); if (!p) return;
  var bd = xiuxianCombatBreakdown(p);
  var rows = '<div class="x-combat-row"><span>境界基础</span><b>' + bd.base + '</b><small>' + bd.baseLabel + '</small></div>' +
  ...
  var html = '<div class="x-modal-head"><span class="x-modal-title">⚔ 战力构成 · ' + escapeHtml((state.students.find(function(s){return s.id===id;})||{}).name||'同学') + '</span><button class="btn btn-sm" data-click="xiuxianCloseModal">✕</button></div>' +
...
function xiuxianMyRank(id, scope, mode) {
  ...
  var list = (state.students || []).filter(function(x){ return scope === 'grade' || x.classId === s.classId; }).map(function(x){
    var p = state.xiuxian.students[x.id]; if (!p) return null;
    return { id: x.id, p: p };
  }).filter(Boolean);
  ...
  var rank = 0;
  for (var i = 0; i < list.length; i++) { if (list[i].id === id) { rank = i + 1; break; } }
  return { rank: rank, total: list.length };
}
function xiuxianTodayStr() { var d = new Date(); return d.getFullYear() + '-' + (d.getMonth() + 1) + '-' + d.getDate(); }
function xiuxianWeekKey() { var d = new Date(); var one = (d.getDay() + 6) % 7; d.setDate(d.getDate() - one); return d.getFullYear() + 'W' + d.getMonth() + d.getDate(); }
...
function ensureXiuxian() {
  ...
  var cur = state.xiuxian.students;
  (state.students || []).forEach(function (s, i) {
    if (!cur[s.id]) {
      var ch = xiuxianRandomCommonChar();
      var squad = (i % XIUXIAN_RULES.SQUAD_RULES.squadsPerClass) + 1;
      cur[s.id] = xiuxianNewProfile(ch.id, squad);
    } else {
      // 补齐新字段（向后兼容）
      var def = xiuxianNewProfile(cur[s.id].characterId, cur[s.id].squad);
      for (var k in def) { if (def.hasOwnProperty(k) && cur[s.id][k] === undefined) cur[s.id][k] = def[k]; }
...
function xiuxianRecalcLinggenSilent() {
  ensureXiuxian();
  for (var k in state.xiuxian.students) {
...
function xiuxianAutoRoutine() {
  var meta = state.xiuxian.meta;
  xiuxianAutoCultivate();
  xiuxianDailyStoneToSpirit();
  var wk = xiuxianWeekKey();
...
function xiuxianAutoSyncHw() {
  ...
  var meta = state.xiuxian.meta;
  var now = Date.now();
  var lastTs = meta.lastHwSyncTs || 0;
  // 检查是否有未领取的作业灵石
  var claimed = state.xiuxian.hwClaimed || {};
  var hasUnclaimed = (state.homeworkRecords || []).some(function(r){
    if (r.status === 'pending') return false;
    var key = r.studentId + '_' + r.taskId;
    return !claimed[key];
  });
  if (!hasUnclaimed) { meta.lastHwSyncTs = now; return; }
  // 12小时无操作自动同步
  var AUTO_SYNC_MS = 12 * 3600000;
  if (now - lastTs < AUTO_SYNC_MS) return;
  // 执行静默自动同步（不弹toast、不renderPage）
  var fc = '';
  var hw = (state.homeworkRecords || []).filter(function(r){
    if (r.status === 'pending') return false;
    return true;
  });
  var total = 0, cnt = 0, premiumGot = 0;
  var cfg = XIUXIAN_RULES.PREMIUM_STONE;
  hw.forEach(function(r){
    var key = r.studentId + '_' + r.taskId;
    if (claimed[key]) return;
    var stones = HW_STONE_RULES[r.status] || 0;
    if (stones > 0 && state.xiuxian.students[r.studentId]) {
      state.xiuxian.students[r.studentId].stone += stones;
      total += stones; cnt++;
      var p = state.xiuxian.students[r.studentId];
      var inWindow = (now - (p.lastPremiumTs || 0)) < cfg.pityWindowDays * 86400000;
      var pity = inWindow ? (p.pityCount || 0) : 0;
      var guaranteed = pity >= cfg.pityThreshold;
      var rate = guaranteed ? 1 : Math.min(1, cfg.dropBaseRate + pity * cfg.dropRateIncrement);
...
function xiuxianWeeklyRoutineAll() {
  var perClass = XIUXIAN_RULES.SQUAD_RULES.leaderWeeklyStone;
  var cnt = 0;
  for (var k in state.xiuxian.students) {
...
function xiuxianWeeklyRoutine(cls) {
  ...
  (state.students || []).forEach(function (s) {
    ...
    var p = state.xiuxian.students[s.id];
...
function xiuxianAutoCultivate() {
  ensureXiuxian();
  var t = xiuxianTodayStr();
  var meta = state.xiuxian.meta;
  if (meta.lastCultivateDay === t) return;
  var ratio = XIUXIAN_RULES.AUTO_SPIRIT_TO_STONE_RATIO || 5;
  var advanced = 0, converted = 0;
  for (var k in state.xiuxian.students) {
    if (!state.xiuxian.students.hasOwnProperty(k)) continue;
    var p = state.xiuxian.students[k];
    if (p.realm >= 6) continue;
    var lg = xiuxianLinggenObj(p.linggen);
    var cap = Math.max(xiuxianRealmObj(p.realm).spiritCap, 10);
    var gain = lg.dailySpirit + (p.gongfaBonus || 0);
    if (p.doubleCultivate) { gain *= 2; p.doubleCultivate = false; }
    // 自动修炼开关：关闭后每日灵气直接转灵石
    if (p.autoCultivate === false) {
      var totalOff = (p.spiritOverflow || 0) + gain;
      var stonesOff = Math.floor(totalOff / ratio);
...
function xiuxianTrySmallBreakthrough(p) {
  ...
  var converted = 0;
  if (p.spirit > cap) {
    var total = (p.spiritOverflow || 0) + (p.spirit - cap);
    var ratio = XIUXIAN_RULES.AUTO_SPIRIT_TO_STONE_RATIO || 3;
    var stones = Math.floor(total / ratio);
...
function xiuxianDailyStoneToSpirit() {
  ...
  var rate = XIUXIAN_RULES.STONE_TO_SPIRIT_RATE || 3;
  for (var k in state.xiuxian.students) {
    if (!state.xiuxian.students.hasOwnProperty(k)) continue;
    var p = state.xiuxian.students[k];
    ...
    var need = Math.max(0, Math.floor((cap - p.spirit) / rate));
    var use = Math.min(need, p.stone);
...
function xiuxianCultivateInfo(id) {
  ...
  var gain = lg.dailySpirit + (p.gongfaBonus || 0);
  var stage = xiuxianEvoStage(p);
  var atMax = (p.stage || 0) >= 2 && p.spirit >= cap;
  var msg = '自动修炼：每日灵气 +' + gain + '（' + lg.displayLabel + '）';
...
function xiuxianExchange(id) {
  var p = xiuxianProfile(id); if (!p) return;
  var rate = XIUXIAN_RULES.STONE_TO_SPIRIT_RATE;
  var cap = Math.max(xiuxianRealmObj(p.realm).spiritCap, 10);
  var maxCan = Math.floor((cap - p.spirit) / rate);
  var maxStones = Math.min(p.stone, Math.max(maxCan, 0));
  var html = '<div class="x-modal-head"><span class="x-modal-title">💱 灵石兑灵气 · ' + escapeHtml((state.students.find(function(s){return s.id===id;})||{}).name||'同学') + '</span><button class="btn btn-sm" data-click="xiuxianCloseModal">✕</button></div>' +
  ...
  xiuxianModal(html);
  setTimeout(function(){
    var inp = document.getElementById('xExAmount');
...
function xiuxianRenderGear(id) {
  ...
  var s = (state.students || []).find(function(x){return x.id===id;})||{};
  var tierColors = ['#9E9E9E','#4CAF50','#2196F3','#AB47BC','#FF7043','#FFD700'];
  var tabBar = '<div class="x-wp-tabs"><span class="x-wp-tab ' + (xiuxianGearTab==='weapon'?'on':'') + '" data-click="xiuxianGearSetTab" data-click-args="' + escapeAttr(JSON.stringify(['' + escapeHtml(id) + '','weapon'])) + '">⚔ 兵器</span>' +
  ...
  if (xiuxianGearTab === 'weapon') {
    var owned = p.weaponInventory || [];
    var curWeapon = p.equipWeapon ? xiuxianWeaponObj(p.equipWeapon) : null;
    var ownedList = owned.map(function(wid){
      var w = xiuxianWeaponObj(wid); if(!w) return '';
      var equipped = p.equipWeapon === wid;
    ...
    var shopList = XIUXIAN_RULES.WEAPONS.filter(function(w){ return owned.indexOf(w.id) < 0; }).map(function(w){
      var costStr = w.costType === 'premium' ? ('💎 ' + w.cost) : ('💰 ' + w.cost);
      var canAfford = w.costType === 'premium' ? p.premium >= w.cost : p.stone >= w.cost;
    ...
    var oOwned = p.outfitInventory || [];
    var curOutfit = p.equipOutfit ? xiuxianOutfitObj(p.equipOutfit) : null;
    var ownedOutfitList = oOwned.map(function(oid){
      var o = xiuxianOutfitObj(oid); if(!o) return '';
      var equipped = p.equipOutfit === oid;
    ...
    var shopOutfitList = XIUXIAN_RULES.OUTFITS.filter(function(o){ return oOwned.indexOf(o.id) < 0; }).map(function(o){
      var costStr = o.costType === 'premium' ? ('💎 ' + o.cost) : ('💰 ' + o.cost);
      var canAfford = o.costType === 'premium' ? p.premium >= o.cost : p.stone >= o.cost;
  ...
  var html = '<div class="x-modal-head"><span class="x-modal-title">🎒 装备库 · ' + escapeHtml(s.name||'同学') + '</span><button class="btn btn-sm" data-click="xiuxianCloseModal">✕</button></div>' +
...
function xiuxianExchangePremium(id) {
  var p = xiuxianProfile(id); if (!p) return;
  var need = XIUXIAN_RULES.PREMIUM_TO_STONE;
...
function xiuxianDrawLimited(id, charId) {
  var p = xiuxianProfile(id); if (!p) return;
  var ch = xiuxianGetChar(charId);
  if (ch.rarity !== 'limited') { xiuxianSwitchChar(id); return; }
  var cls = ((state.students.find(function (s) { return s.id === id; })) || {}).classId || '';
  var mates = Object.keys(state.xiuxian.students).filter(function (sid) {
  ...
  var rank = mates.indexOf(id) + 1;
  if (ch.rank_req && rank > ch.rank_req) { showToast('需班级排名前' + ch.rank_req + '（当前第' + rank + '名）', 'warn'); return; }
  var taken = 0; for (var k in state.xiuxian.students) { if (state.xiuxian.students[k].characterId === charId) taken++; }
...
function xiuxianOpenBreakthrough(id) {
  ...
  var costs = XIUXIAN_RULES.BREAKTHROUGH_COSTS[p.realm + 1];
  var baseRates = XIUXIAN_RULES.BREAKTHROUGH_BASE_RATES;
  var bonus = XIUXIAN_RULES.BREAKTHROUGH_FAIL_BONUSES;
  var fails = p.btFails || 0;
  var rows = '';
  for (var t = 1; t <= 4; t++) {
    var rate = Math.min(1, baseRates[t - 1] + fails * bonus[t - 1]);
    var guar = (t === 4) ? '（失败必成）' : '';
  ...
  var html = '<div class="x-modal-head"><span class="x-modal-title">⚡ 突破 · ' + xiuxianRealmObj(p.realm).name + ' → ' + xiuxianRealmObj(p.realm + 1).name + '</span><button class="btn btn-sm" data-click="xiuxianCloseModal">✕</button></div>' +
...
function xiuxianBreakthrough(id, tier) {
  ...
  var rate = Math.min(1, XIUXIAN_RULES.BREAKTHROUGH_BASE_RATES[tier - 1] + (p.btFails || 0) * XIUXIAN_RULES.BREAKTHROUGH_FAIL_BONUSES[tier - 1]);
  if (p.stone < cost) { showToast('突破需 ' + cost + ' 灵石（当前 ' + p.stone + '），努力修炼吧', 'warn'); xiuxianCloseModal(); return; }
  p.stone -= cost;
  var success = Math.random() < rate || tier === 4;
  if (success) {
...
function xiuxianTaskReward(id) {
  ...
  var guaranteed = pity >= cfg.pityThreshold;
  var rate = guaranteed ? 1 : Math.min(1, cfg.dropBaseRate + pity * cfg.dropRateIncrement);
  var hit = Math.random() < rate || guaranteed;
  if (hit) {
...
function xiuxianRenderMall(id) {
  var p = xiuxianProfile(id); if (!p) return;
  var cats = [['gongfa', '功法'], ['dan', '丹药'], ['xiulian', '修炼道具']];
  var catBar = cats.map(function (c) { return '<span class="x-wp-tab ' + (c[0] === xiuxianMallCat ? 'on' : '') + '" data-click="xiuxianMallSetCat" data-click-args="' + escapeAttr(JSON.stringify(['' + escapeHtml(id) + '','' + c[0] + ''])) + '">' + c[1] + '</span>'; }).join('');
  var items = XIUXIAN_RULES.MALL_CATALOG.filter(function (it) { return it.cat === xiuxianMallCat; });
  var grid = items.map(function (it) {
    var costStr = it.costType === 'premium' ? ('💎 ' + it.cost) : ('💰 ' + it.cost);
    var e = it.effect;
    var owned = false;
  ...
  var html = '<div class="x-modal-head"><span class="x-modal-title">🛒 修炼商城 · ' + escapeHtml((state.students.find(function (s) { return s.id === id; }) || {}).name || '同学') + '</span><button class="btn btn-sm" data-click="xiuxianCloseModal">✕</button></div>' +
...
function xiuxianBuyMall(id, itemId) {
  var p = xiuxianProfile(id); if (!p) return;
  var it = XIUXIAN_RULES.MALL_CATALOG.find(function (x) { return x.id === itemId; }); if (!it) return;
  var cost = it.cost;
...
// —— 排行榜（Stage 5） ——
var xiuxianRankMode = 'combat', xiuxianRankAnon = false;
...
function xiuxianRenderRank() {
  var cls = state.xiuxian._rankCls || '';
  var list = (state.students || []).filter(function (s) { return !cls || s.classId === cls; }).map(function (s) {
    var p = state.xiuxian.students[s.id]; if (!p) return null;
  ...
  list.forEach(function (o, i) {
    ...
    var nm = xiuxianRankAnon ? ('匿名' + rankIdx) : escapeHtml(o.name || '同学');
    var val = xiuxianRankMode === 'combat' ? xiuxianCombat(o.p) : o.p.stone;
    var ch = xiuxianGetChar(o.p.characterId);
    var frameCls = rankIdx <= 10 ? ' x-rank-frame-' + rankIdx : '';
    var avatarHtml = '<div class="x-rank-avatar"><div class="x-rank-frame' + frameCls + '">' + xiuxianAvatarSVG(ch, o.p) + '</div></div>';
  ...
  var limitedNote = '<div class="x-tip">限定角色需榜单名次门槛 + 极品灵石抽取（如迪迦奥特曼限前10名）。匿名模式仅隐藏姓名，名次与数值不变。</div>';
  var html = '<div class="x-modal-head"><span class="x-modal-title">🏆 修仙排行榜' + (cls ? ' · ' + escapeHtml(cls) : ' · 全年级') + '</span><button class="btn btn-sm" data-click="xiuxianCloseModal">✕</button></div>' +
...
function xiuxianOpenTeam(cls) {
  var n = XIUXIAN_RULES.SQUAD_RULES.squadsPerClass;
  var grids = '';
  for (var sq = 1; sq <= n; sq++) {
    var members = (state.students || []).filter(function (s) { return s.classId === cls && state.xiuxian.students[s.id] && state.xiuxian.students[s.id].squad === String(sq); });
    var mhtml = members.map(function (s) {
      var p = state.xiuxian.students[s.id];
      var isL = p && p.isLeader;
  ...
  var html = '<div class="x-modal-head"><span class="x-modal-title">👥 ' + escapeHtml(cls) + ' 小队管理（' + n + ' 队）</span><button class="btn btn-sm" data-click="xiuxianCloseModal">✕</button></div>' +
...
function xiuxianCloseModal() { var o = document.getElementById('x-modal-overlay'); if (o) o.remove(); }

// —— 渲染 ——
function renderXiuxian(area) {
  ensureXiuxian(); xiuxianAutoRoutine();
  var v = state.xiuxian.view || 'archive';
...
function xiuxianTabBar(active) {
  var tabs = [{id:'archive',icon:'📖',label:'01修仙档案总库'},{id:'tasks',icon:'📜',label:'02修仙任务系统'},{id:'rank',icon:'🏆',label:'03修仙排行榜单'},{id:'pool',icon:'🎴',label:'04修仙角色体质'}];
...
function renderXiuxianArchive(area) {
  ...
  var cset = {}; all.forEach(function(s){ if(s.classId) cset[s.classId]=1; });
  var classes = Object.keys(cset).sort();
  var fc = state.xiuxian._filterClass || '', fl = state.xiuxian._filterLinggen || '', sk = state.xiuxian._searchKey || '';
  var filtered = all.filter(function(s){
    if (fc && s.classId !== fc) return false;
    if (fl) { var p = xiuxianProfile(s.id); if (p.linggen !== fl) return false; }
    if (sk) { var n = (s.name||'').toLowerCase(), no = (s.studentNo||'').toLowerCase(); if (n.indexOf(sk.toLowerCase())<0 && no.indexOf(sk.toLowerCase())<0) return false; }
    return true;
  });
  var classOpts = '<option value="">全部班级</option>' + classes.map(function(c){return '<option value="'+escapeHtml(c)+'"'+(fc===c?' selected':'')+'>'+escapeHtml(c)+'</option>';}).join('');
  var lgOpts = '<option value="">全部灵根</option>' + XIUXIAN_RULES.LINGGEN_LEVELS.map(function(l){return '<option value="'+l.name+'"'+(fl===l.name?' selected':'')+'>'+l.displayLabel+'</option>';}).join('');
  var cards = filtered.map(function(s){
    var p = xiuxianProfile(s.id); var ch = xiuxianGetChar(p.characterId); var lg = xiuxianLinggenObj(p.linggen); var realm = xiuxianRealmObj(p.realm);
    var cp = xiuxianCombat(p);
...
function renderXiuxianClassHome(area, cls) {
  ...
  var mates = (state.students || []).filter(function (s) { return s.classId === cls; });
  var cards = mates.map(function (s) {
    var p = xiuxianProfile(s.id); var ch = xiuxianGetChar(p.characterId); var lg = xiuxianLinggenObj(p.linggen);
    var cp = xiuxianCombat(p);
    return '<div class="x-card x-stu-card '+xiuxianLinggenFrameClass(p.linggen)+'" data-click="xiuxianSelectStudent" data-click-args="' + escapeAttr(JSON.stringify(['' + escapeHtml(s.id) + ''])) + '">' +
      '<div class="x-cell-avatar x-act-' + ch.action + '" style="filter:drop-shadow(0 0 6px ' + lg.glowColor + ')">' + xiuxianAvatarSVG(ch, p) + '</div>' +
      '<div class="x-stu-name">' + escapeHtml(s.name || '同学') + '</div>' +
  ...
  var tSpirit = mates.reduce(function(a,s){var p=xiuxianProfile(s.id);return a+(p.spirit||0);},0);
  var tStone = mates.reduce(function(a,s){var p=xiuxianProfile(s.id);return a+(p.stone||0);},0);
  var tCombat = mates.reduce(function(a,s){var p=xiuxianProfile(s.id);return a+xiuxianCombat(p);},0);
  var tLeader = mates.filter(function(s){return xiuxianProfile(s.id).isLeader;}).length;
  var tRealm = mates.reduce(function(a,s){return a+(xiuxianProfile(s.id).realm||0);},0);
  area.innerHTML = '<div class="x-wrap">' +
    '<div class="x-mist top"></div>' +
...
function renderXiuxianStudentHome(area, id) {
  ...
  var p = xiuxianProfile(id);
  var ch = xiuxianGetChar(p.characterId);
  var lg = xiuxianLinggenObj(p.linggen);
  var realm = xiuxianRealmObj(p.realm);
  var cap = Math.max(realm.spiritCap, 10);
  var pct = cap > 0 ? Math.min(100, Math.round(p.spirit / cap * 100)) : 100;
  var stage = xiuxianEvoStage(p);
  var switchLabel = p.switchUsed ? ('更换角色(灵石' + XIUXIAN_RULES.SWITCH_COST_STONE + ')') : '更换角色(免费)';
  var wName = p.equipWeapon ? (xiuxianWeaponObj(p.equipWeapon) || {}).name : '无';
  var oName = p.equipOutfit ? (xiuxianOutfitObj(p.equipOutfit) || {}).name : '无';
  var cp = xiuxianCombat(p);
  var atMaxStage = (p.stage || 0) >= 2 && p.spirit >= cap;
  var btReady = atMaxStage && p.realm < 6;
  var stageDots = '';
  for (var si = 0; si < 3; si++) { stageDots += '<span class="x-stage-dot' + (si <= (p.stage || 0) ? ' on' : '') + '"></span>'; }
  var autoCultOn = p.autoCultivate !== false;
  var autoConvertOn = p.autoStoneConvert !== false;
  var rCombatC = xiuxianMyRank(id, 'class', 'combat');
  var rCombatG = xiuxianMyRank(id, 'grade', 'combat');
  var rRealmC = xiuxianMyRank(id, 'class', 'realm');
  var rRealmG = xiuxianMyRank(id, 'grade', 'realm');
  var rPremiumC = xiuxianMyRank(id, 'class', 'premium');
  var rPremiumG = xiuxianMyRank(id, 'grade', 'premium');
  var rankMeHtml = '<div class="x-rank-me">' +
...
// ===== ② 修仙任务系统 =====
var HW_STONE_RULES = { excellent: 5, normal: 3, resubmitted: 2, perfunctory: 1, incomplete: 0, pending: 0 };
var HW_STATUS_LABELS = { excellent: '优秀', normal: '正常', resubmitted: '补交', perfunctory: '敷衍', incomplete: '未交', pending: '待标记' };

function xiuxianTaskFilterClass(cls) { state.xiuxian._taskClass = cls; saveState(); renderPage(); }

function xiuxianSyncHwStones() {
  ensureXiuxian();
  var fc = state.xiuxian._taskClass || '';
  var hw = (state.homeworkRecords || []).filter(function(r){
  ...
  saveState(); renderPage();
  if (cnt > 0) {
    var msg = '作业灵石同步：' + cnt + ' 人共 +' + total + ' 灵石';
...
function xiuxianToggleMulti(sid) {
  ...
  var cnt = Object.keys(_xMultiSelect).length;
  var bar = document.getElementById('xMultiBar');
...
function xiuxianMultiAward(type) {
  ensureXiuxian();
  var typeEl = document.getElementById('xMultiType');
  var stoneEl = document.getElementById('xMultiStone');
  var descEl = document.getElementById('xMultiDesc');
  var stoneAmt = parseInt(stoneEl ? stoneEl.value : '3') || 0;
  if (stoneAmt <= 0) { showToast('灵石数量必须大于0', 'warn'); return; }
  var desc = descEl ? (descEl.value || '') : '';
  var taskType = type === 'other' ? '其它' : (typeEl ? typeEl.value : '背诵');
  var ids = Object.keys(_xMultiSelect);
  if (ids.length === 0) { showToast('请先勾选学生', 'warn'); return; }
  ids.forEach(function(sid){
    if (state.xiuxian.students[sid]) state.xiuxian.students[sid].stone += stoneAmt;
  });
  var fc = state.xiuxian._taskClass || '';
  var rec = { id: 'ct' + Date.now(), type: taskType, desc: desc || taskType, stone: stoneAmt, classId: fc, date: xiuxianTodayStr(), studentIds: ids, studentCount: ids.length };
  if (!state.xiuxian.customTasks) state.xiuxian.customTasks = [];
...
function xiuxianAwardOne() {
  ensureXiuxian();
  var sidEl = document.getElementById('xIndStudent');
  var stoneEl = document.getElementById('xIndStone');
  var descEl = document.getElementById('xIndDesc');
  var typeEl = document.getElementById('xIndType');
  var sid = sidEl ? sidEl.value : '';
  var stoneAmt = parseInt(stoneEl ? stoneEl.value : '3') || 0;
  if (!sid) { showToast('请选择学生', 'warn'); return; }
  if (stoneAmt <= 0) { showToast('灵石数量必须大于0', 'warn'); return; }
  var desc = descEl ? (descEl.value || '') : '';
  var taskType = typeEl ? typeEl.value : '背诵';
  var s = (state.students || []).find(function(x){ return x.id === sid; });
  if (!s) { showToast('未找到学生', 'warn'); return; }
  if (state.xiuxian.students[sid]) { state.xiuxian.students[sid].stone += stoneAmt; }
  var rec = { id: 'ct' + Date.now(), type: taskType, desc: desc || taskType, stone: stoneAmt, classId: s.classId || '', date: xiuxianTodayStr(), studentIds: [sid], studentCount: 1 };
  if (!state.xiuxian.customTasks) state.xiuxian.customTasks = [];
  state.xiuxian.customTasks.unshift(rec);
  if (state.xiuxian.customTasks.length > 100) state.xiuxian.customTasks = state.xiuxian.customTasks.slice(0, 100);
  saveState(); renderPage();
...
function renderXiuxianTasks(area) {
  ensureXiuxian();
  var cset = {}; (state.students || []).forEach(function(s){ if(s.classId) cset[s.classId]=1; });
  var classes = Object.keys(cset).sort();
  var fc = state.xiuxian._taskClass || '';
  var classOpts = '<option value="">全部班级</option>' + classes.map(function(c){return '<option value="'+escapeHtml(c)+'"'+(fc===c?' selected':'')+'>'+escapeHtml(c)+'</option>';}).join('');
  ...
  var stats = { excellent:0, normal:0, resubmitted:0, perfunctory:0, incomplete:0 };
  var unclaimed = 0, totalStones = 0;
  var claimed = state.xiuxian.hwClaimed || {};
  hw.forEach(function(r){
  ...
  var customs = state.xiuxian.customTasks || [];
  ...
  var fc2 = fc;
  var students = (state.students || []).filter(function(s){ return !fc2 || s.classId === fc2; });
  var studentGrid = students.map(function(s){
    var p = xiuxianProfile(s.id); var lg = xiuxianLinggenObj(p.linggen); var realm = xiuxianRealmObj(p.realm);
    var checked = _xMultiSelect[s.id] ? ' checked' : '';
    var totalStones = 0;
    var types = byStudent[s.id] ? byStudent[s.id].types : {};
    for (var k in types) totalStones += types[k];
  ...
  var summaryList = Object.keys(byStudent).map(function(sid){ return { sid:sid, ...byStudent[sid] }; });
  summaryList.sort(function(a,b){
    var ta = 0, tb = 0;
    for (var k in a.types) ta += a.types[k];
    for (var k in b.types) tb += b.types[k];
    return tb - ta;
  });
  if (summaryList.length > 0) {
    var header = '<tr class="x-sum-head"><td>学生</td><td>班级</td>' + typeList.map(function(t){ return '<td>' + escapeHtml(t) + '</td>'; }).join('') + '<td>合计</td></tr>';
    var rows = summaryList.map(function(o){
      var total = 0;
      var cells = typeList.map(function(t){ var v = o.types[t]||0; total += v; return '<td>' + (v||'-') + '</td>'; }).join('');
...
// ===== ③ 角色池 =====
var _xPoolCat = 'all', _xPoolDrawStudent = '';
...
function renderXiuxianPool(area) {
  ensureXiuxian();
  var cats = [{id:'all',label:'全部'},{id:'animal',label:'动物池'},{id:'plant',label:'植物池'},{id:'character',label:'动漫角色'},{id:'rare',label:'稀有角色'}];
  var pool = XIUXIAN_POOL.filter(function(c){ return _xPoolCat==='all' || c.pool===_xPoolCat; });
  var cards = pool.map(function(c){
    var p = xiuxianNewProfile(c.id, 1);
    var lg = xiuxianLinggenObj(p.linggen);
    var isRare = c.rarity === 'limited';
  ...
  var drawStudentOpts = '<option value="">选择学生</option>' + (state.students||[]).map(function(s){return '<option value="'+escapeHtml(s.id)+'"'+(_xPoolDrawStudent===s.id?' selected':'')+'>'+escapeHtml(s.name||'同学')+' · '+escapeHtml(s.classId||'')+'</option>';}).join('');
  var limitedBtns = '';
  if (_xPoolDrawStudent) {
    var dp = xiuxianProfile(_xPoolDrawStudent);
    if (dp) {
      var dch = xiuxianGetChar(dp.characterId);
...
// ===== ⑤ 排行榜标签页 =====
var _xRankPageMode = 'combat', _xRankPageAnon = false, _xRankPageCls = '', _xRankPageLinggen = '', _xRankPageSelf = '', _xRankPageScope = 'grade';
...
function renderXiuxianRankPage(area) {
  ...
  var cset = {}; (state.students || []).forEach(function(s){ if(s.classId) cset[s.classId]=1; });
  var classes = Object.keys(cset).sort();
  var scope = _xRankPageScope || 'grade';
  var cls = scope === 'class' ? (_xRankPageCls || classes[0] || '') : '';
  var lgf = _xRankPageLinggen || '';
  var selfId = _xRankPageSelf || '';
  var list = (state.students || []).filter(function(s){ return !cls || s.classId === cls; }).map(function(s){
    var p = state.xiuxian.students[s.id]; if (!p) return null;
  ...
  var selfRank = -1, selfName = '';
  list.forEach(function(o, i) { if (o.id === selfId) { selfRank = i + 1; selfName = o.name; } });
  var modeLabel = _xRankPageMode === 'combat' ? '战力' : _xRankPageMode === 'realm' ? '境界' : '极品灵石';
  var scopeLabel = scope === 'class' ? (cls + '班级榜') : '年级总榜';
  ...
  list.forEach(function(o, i) {
    ...
    var rankIdx = i + 1;
    var medal = i === 0 ? '🥇' : i === 1 ? '🥈' : i === 2 ? '🥉' : rankIdx;
    var nm = _xRankPageAnon ? ('匿名' + rankIdx) : escapeHtml(o.name || '同学');
    var val = valOf(o.p);
    var ch = xiuxianGetChar(o.p.characterId);
    var frameCls = rankIdx <= 10 ? ' x-rank-frame-' + rankIdx : '';
    var avatarHtml = '<div class="x-rank-avatar"><div class="x-rank-frame' + frameCls + '">' + xiuxianAvatarSVG(ch, o.p) + '</div></div>';
  ...
  var classOpts = classes.map(function(c){return '<option value="'+escapeHtml(c)+'"'+(cls===c?' selected':'')+'>'+escapeHtml(c)+'</option>';}).join('');
  var lgOpts = '<option value="">全部灵根</option>' + XIUXIAN_RULES.LINGGEN_LEVELS.map(function(l){return '<option value="'+l.name+'"'+(lgf===l.name?' selected':'')+'>'+l.displayLabel+'</option>';}).join('');
  var selfOpts = '<option value="">选择查看自己排名</option>' + (state.students||[]).filter(function(s){return !cls||s.classId===cls;}).map(function(s){return '<option value="'+escapeHtml(s.id)+'"'+(selfId===s.id?' selected':'')+'>'+escapeHtml(s.name||'同学')+'</option>';}).join('');
  var selfBanner = selfRank > 0 ? '<div class="x-self-rank">🏆 ' + escapeHtml(selfName) + ' 在' + scopeLabel + modeLabel + '榜排名第 ' + selfRank + ' 名（共' + list.length + '人）</div>' : '';
  area.innerHTML = '<div class="x-wrap">' +
    '<div class="x-mist top"></div>' +
...
if (!document.getElementById('xiuxian-style2')) {
  var _xs2 = document.createElement('style');
...
function __dcRemoveParent(args, e, el) { var tgt = el || (e && e.target); if(tgt && tgt.parentElement) tgt.parentElement.remove(); }
...
const __EV = {
  ev70: function ev70(el, e) {
    ...
    var m = el.id.match(/^cloud-(up|dl)-cls-(.+)-(.+)$/);
    if (!m) return;
    var which = m[1], secKey = m[2], cls = m[3];
    var field = which === 'up' ? 'cloudUploadSectionClasses' : 'cloudDownloadSectionClasses';
    var map = state[field] || {};
    var arr = map[secKey] || [];
  ...
  ev71: function ev71(el, e) {
    var m = el.id.match(/^cloud-(up|dl)-(.+)$/);
    if (!m) return;
    var prefix = m[1];
    var sections = cloudGetCheckedSections(prefix);
  ...
  ev11: function ev11(el, e, a1) { var event = e; toggleTaskComplete(a1); },
  ev12: function ev12(el, e) { var event = e; event.preventDefault();this.classList.add('drag-over') },
  ev13: function ev13(el, e) { var event = e; this.classList.remove('drag-over') },
  ev14: function ev14(el, e) { var event = e; onQuadrantDrop(event, this) },
  ev15: function ev15(el, e, a1) { var event = e; event.dataTransfer.setData('text/plain', a1);this.classList.add('dragging') },
  ev16: function ev16(el, e) { var event = e; this.classList.remove('dragging') },
  ev17: function ev17(el, e) { var event = e; event.preventDefault();this.classList.add('dragover') },
  ev18: function ev18(el, e) { var event = e; this.classList.remove('dragover') },
  ev19: function ev19(el, e) { var event = e; handlePlanDrop(event) },
  ev2: function ev2(el, e) { var event = e; handleImportFile(this) },
  ev20: function ev20(el, e) { var event = e; handlePlanUpload(this) },
  ev21: function ev21(el, e) { var event = e; handleScheduleUpload(event) },
  ev22: function ev22(el, e) { var event = e; jumpToWeek(this.value) },
  ev23: function ev23(el, e, a1) { var event = e; onPeriodTypeChange(a1); },
  ev24: function ev24(el, e, a1) { var event = e; onPeriodTypeChange(a1); },
  ev25: function ev25(el, e) { var event = e; onClassFilterChange(this.value) },
  ev26: function ev26(el, e) { var event = e; onStudentSearch(this.value) },
  ev27: function ev27(el, e) { var event = e; onStudentSearch(document.getElementById('studentSearch').value) },
  ev28: function ev28(el, e) { var event = e; handleStudentUpload(this) },
  ev29: function ev29(el, e) { var event = e; handleTaskInfoUpload(this) },
  ev3: function ev3(el, e) { var event = e; handleMergeFile(this) },
  ev30: function ev30(el, e, a1) { var event = e; handleTaskImageUpload(event, a1); },
  ev31: function ev31(el, e) { var event = e; _debouncedRenderHomeworkTable() },
  ev32: function ev32(el, e) { var event = e; onHwClassFilterChange() },
  ev33: function ev33(el, e) { var event = e; onHwTaskFilterChange() },
  ev34: function ev34(el, e) { var event = e; renderHomeworkTable() },
  ev35: function ev35(el, e) { var event = e; toggleSelectAllHw(this.checked) },
  ev36: function ev36(el, e, a1) { var event = e; toggleHwSelection(a1); },
  ev37: function ev37(el, e) { var event = e; if(event.key==='Enter')sendChat() },
  ev38: function ev38(el, e) { var event = e; if(event.key==='Enter')verifyChatUnlock() },
  ev39: function ev39(el, e) { var event = e; if(event.key==='Enter')verifyTeacherReply() },
  ev4: function ev4(el, e) { var event = e; if(event.key==='Enter')document.getElementById('pwdSet2').focus() },
  ev40: function ev40(el, e, a1) { var event = e; if(event.key==='Enter')verifyTplPwd(a1) },
  ev41: function ev41(el, e, a1) { var event = e; if(event.key==='Enter')verifySpecialPwd(a1) },
  ev42: function ev42(el, e) { var event = e; state._hwAnalysisClass=this.value;saveState();renderHwAnalysis(document.getElementById('analysisContainer')) },
  ev43: function ev43(el, e) { var event = e; state._hwAnalysisTask=this.value;saveState();renderHwAnalysis(document.getElementById('analysisContainer')) },
  ev44: function ev44(el, e) { var event = e; state._dashboardClass=this.value;saveState();renderDashboard(document.getElementById('analysisContainer')) },
  ev45: function ev45(el, e) { var event = e; state._selectedExam=this.value;saveState();renderDashboard(document.getElementById('analysisContainer')) },
  ev46: function ev46(el, e) { var event = e; state._dashboardKeyClassFilter=this.value;saveState();renderDashboard(document.getElementById('analysisContainer')) },
  ev47: function ev47(el, e, a1) { var event = e; toggleKeyTypeFilter(a1); },
  ev48: function ev48(el, e) { var event = e; state._selectedExam=this.value;saveState();renderScoreAnalysis(document.getElementById('analysisContainer')) },
  ev49: function ev49(el, e) { var event = e; state._scoreClassFilter=this.value;saveState();renderScoreAnalysis(document.getElementById('analysisContainer')) },
  ev5: function ev5(el, e) { var event = e; if(event.key==='Enter')document.getElementById('secQuestion').focus() },
  ev50: function ev50(el, e) { var event = e; handleScoreUpload(this) },
  ev51: function ev51(el, e) { var event = e; state._scoreShowRegress=this.checked;saveState();renderScoreAnalysis(document.getElementById('analysisContainer')) },
  ev52: function ev52(el, e) { var event = e; state._scoreShowVolatile=this.checked;saveState();renderScoreAnalysis(document.getElementById('analysisContainer')) },
  ev53: function ev53(el, e) { var event = e; state._scoreShowSteady=this.checked;saveState();renderScoreAnalysis(document.getElementById('analysisContainer')) },
  ev54: function ev54(el, e) { var event = e; state._scoreShowTop=this.checked;saveState();renderScoreAnalysis(document.getElementById('analysisContainer')) },
  ev55: function ev55(el, e) { var event = e; state._scoreShowProgress=this.checked;saveState();renderScoreAnalysis(document.getElementById('analysisContainer')) },
  ev56: function ev56(el, e) { var event = e; onReminderFreqChange() },
  ev57: function ev57(el, e) { var event = e; xiuxianArchiveFilter('class',this.value) },
  ev58: function ev58(el, e) { var event = e; xiuxianArchiveFilter('linggen',this.value) },
  ev59: function ev59(el, e) { var event = e; xiuxianArchiveFilter('search',this.value) },
  ev6: function ev6(el, e, a1) { var event = e; if(event.key==='Enter')confirmSetPassword(a1) },
  ev60: function ev60(el, e, a1) { var event = e; xiuxianToggleMulti(a1); },
  ev61: function ev61(el, e) { var event = e; xiuxianTaskFilterClass(this.value) },
  ev62: function ev62(el, e) { var event = e; xiuxianPoolSetDrawStudent(this.value) },
  ev63: function ev63(el, e) { var event = e; xiuxianRankPageSetClass(this.value) },
  ev64: function ev64(el, e) { var event = e; xiuxianRankPageSetLinggen(this.value) },
  ev65: function ev65(el, e) { var event = e; xiuxianRankPageSetSelf(this.value) },
  ev7: function ev7(el, e, a1) { var event = e; if(event.key==='Enter')verifyPassword(a1) },
  ev8: function ev8(el, e) { var event = e; if(event.key==='Enter')verifySecurityAnswer() },
  ev9: function ev9(el, e) { var event = e; if(event.key==='Enter')unlockFromOverlay() }
};

function __parseEvArgs(el) {
  var raw = el.getAttribute('data-ev-args');
  if (!raw) return [];
  ...
  document.addEventListener(t, function(e) {
    ...
    while (el && el !== document) {
      if (el.hasAttribute && el.hasAttribute('data-ev')) {
        var evTypes = el.getAttribute('data-ev');
        var typeArr = evTypes.split('|');
        var idx = typeArr.indexOf(t);
        if (idx !== -1) {
          var keyStr = el.getAttribute('data-ev-key') || '';
          var key = keyStr.split('|')[idx];
          var fn = __EV[key];
          if (typeof fn !== 'function') { console.warn('[delegate:' + t + '] 未找到处理器', key); return; }
          var args = __parseEvArgs(el);
