...
/// 验证路径是否安全可访问：必须是绝对路径、本地目录、拒绝 UNC 和特殊路径
fn validate_local_dir(path: &Path) -> Result<PathBuf, String> {
    ...
    // 使用 canonicalize 解析真实路径（验证存在性并规范化）
...
// ── 安全封装的 DB 写操作（替代 plugin:sql|execute，移除 sql:allow-execute 权限）──
// 仅允许 INSERT / UPDATE / DELETE，且目标表必须存在于 sqlite_master（迁移创建的表自动放行，
// 未来新功能加表即自动获得写权限，不会被阻塞）。参数化执行，杜绝 SQL 注入。
...
fn db_execute(
    ...
    // 仅允许 INSERT / UPDATE / DELETE，并提取目标表
    ...
    // 打开 DB（与插件只读连接并存；WAL 降低锁竞争）
    let conn = Connection::open(&state.path)
        .map_err(|e| format!("打开数据库失败: {}", e))?;
    let _ = conn.execute("PRAGMA journal_mode=WAL", ());

    // 校验目标表真实存在（迁移创建的表在 sqlite_master 中 → 自动放行，新功能不阻塞）
...
/// S3 配置（存储在后端文件中，避免凭据通过 IPC 明文传递）
...
/// 保存 S3 配置到后端文件（前端调用，凭据不再留在 IPC 通道中反复传递）
#[tauri::command]
fn save_s3_config(
    endpoint: String,
    bucket: String,
...
/// 获取 S3 配置状态（不返回 secret_key，仅返回是否已设置）
#[tauri::command]
...
/// Build a reqwest client with timeouts, no proxy, no default headers
